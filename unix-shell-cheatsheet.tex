...
% Define \key command based on https://tex.stackexchange.com/a/5227
\usepackage{tikz}
...
\newcommand*\key[1]{%
  \tikz[baseline=(key.base)]
    ...
    ](key){\hspace*{2pt}#1\hspace*{2pt}\strut};}
...
\put(190,205){ % Position on the page to put the title
Updated on: \today
}
...
\command{\emph{cmd} <{}<{}< "input line 1"}		{"\emph{here-string}":}
													\null\hfill read string as if read from a file with one line\\
\command{\emph{cmd} <{}< \emph{END}}			{"\emph{here-document}":}
input line 1									\null\hfill read next input-lines until \texttt{END} \\
input line 2									\null\hfill as if lines were read from a file; \\
\texttt{END}									\null\hfill \emph{END} can be any word e.g. \emph{EOF}\\
...
\command{\key{Ctrl} + a  \emph{or} \key{Home}}	{go to start of line}
\command{\key{Ctrl} + e  \emph{or} \key{End}}		{go to end of line}
\command{\key{Ctrl} + k}							{\emph{kill}/clear line to end}
\command{\key{Ctrl} + u}							{\emph{kill}/clear line to start}
\command{\key{Ctrl} + w}							{delete word backwards}
\command{\key{Ctrl} + y}							{\emph{yank} deleted text}
...
\command{\key{Alt} + <}											{jump to first entry in history}
\command{\key{Alt} + \key{Shift} + >}				{jump to prompt (end of hist.)}
\command{\key{Alt} + r}											{search backwards in history; \\
																\null\hfill use again to repeat search}
\command{\key{Ctrl} + g}										{abort history search}
\command{\textbf{history}}									{print all commands in history}
\command{\textbf{history} 10}								{print last 10 commands}
\command{!\emph{100}}												{run command with id \emph{100} from history}
\command{\emph{cmd} !{}!}										{run last command prefixed with cmd}
